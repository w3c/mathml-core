...
for subdir in $WPT_SUBDIRS; do
    ...
    find $WPT_DIR/$subdir -type f -name '*.html' | xargs grep '<link' | grep 'rel="help"' | sed "s|$WPT_DIR/$subdir/\([^:]\+\):.\+https://mathml-refresh.github.io/mathml-core/#\([^\"]\+\)\".\+|add_test('\2','\1')|" >> $PYTHON_SCRIPT
    echo "done"
done
